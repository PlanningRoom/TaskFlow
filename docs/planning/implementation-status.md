...
- `make dev` end-to-end boot of the full docker-compose stack (db + mailhog + api + web).
- `make test` → all integration tests green against a real Postgres (locally requires Docker).
- A green CI run on a real PR (the new `tests` job will exercise B2's migration-boot test and the B4 endpoint tests).
