...
**Decision:** A single AWS EC2 `t4g.small` instance (ARM64, 2 vCPU, 2 GB RAM) in us-east-1 (Decision 037). The instance runs Docker Compose with three containers: the FastAPI app (Uvicorn), PostgreSQL 16, and nginx (reverse proxy + TLS termination + static asset serving). Route 53 provides DNS; an S3 bucket stores backups; Amazon SES sends email (Decision 067).
